  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-white/40">{bookings.length} total booking{bookings.length !== 1 ? 's' : ''}</p>
      </div>
              ...
              bookings.map((b) => {
                ...
                return (
                  <tr key={b.id} className="border-b last:border-0" style={{ borderColor: 'var(--cv-border)' }}>
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/dashboard/bookings/${b.id}`} className="text-xs hover:text-white" style={{ color: 'var(--cv-brand)' }}>
                        View →
                      </Link>
                    </td>
                  </tr>
