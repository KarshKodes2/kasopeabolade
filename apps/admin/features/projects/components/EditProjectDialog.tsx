...
export function EditProjectDialog({ project, onClose }: Props) {
  ...
  const [form, setForm] = useState({
    title: project.title, subtitle: project.subtitle ?? '', slug: project.slug,
    description: project.description ?? '', url: project.url ?? '',
    emoji: project.emoji ?? '', tags: project.tags.join(', '),
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="dialog-content animate-slide-up">
          <div className="mb-5 flex items-center justify-between">
            <Dialog.Title className="text-base font-semibold" style={{ color: 'var(--cv-text)' }}>Edit Project</Dialog.Title>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input className="admin-input" value={form.emoji} onChange={(e) => setForm({ ...form, emoji: e.target.value })} maxLength={2} />
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <input className="admin-input" value={form.url} onChange={(e) => setForm({ ...form, url: e.target.value })} type="url" />
            </div>
            <div>
